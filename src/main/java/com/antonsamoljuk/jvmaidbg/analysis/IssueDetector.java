...
public class IssueDetector {
    ...
    public DetectedIssue detect(ExtractedEvidence evidence, String rawContent) {
        return detect(evidence, rawContent, InputType.AUTO);
    }

    public DetectedIssue detect(ExtractedEvidence evidence, String rawContent, InputType inputType) {
        ...
        IssueCategory category = (custom != null) ? custom : categorize(exceptions, causedBy, lower, evidence, inputType);
    ...
    private IssueCategory categorize(List<String> exceptions, List<String> causedBy,
                                     String lower, ExtractedEvidence evidence, InputType hint) {
        boolean checkExceptions  = hint != InputType.BUILD_LOG;
        boolean checkTestFrameworks = hint != InputType.BUILD_LOG && hint != InputType.STACKTRACE;
        boolean checkBuildTools  = hint != InputType.TEST_FAILURE && hint != InputType.STACKTRACE;

        // Spring context failures (exception-based, relevant for all types)
        if (checkExceptions && (containsAny(exceptions, "BeanCreationException", "UnsatisfiedDependencyException",
        ...
        if (checkExceptions && containsAny(exceptions, "OutOfMemoryError", "StackOverflowError", "GCOverheadLimitExceededError")) {
        ...
        if (checkExceptions && exceptions.contains("NoClassDefFoundError")) {
            return IssueCategory.NO_CLASS_DEF_FOUND;
        }
        if (checkExceptions && exceptions.contains("ClassNotFoundException")) {
            return IssueCategory.CLASS_NOT_FOUND;
        }

        // NullPointerException
        if (checkExceptions && exceptions.contains("NullPointerException")) {
            return IssueCategory.NULL_POINTER_EXCEPTION;
        }

        // Hibernate/JPA
        if (checkExceptions && (containsAny(exceptions, "MappingException", "HibernateException",
        ...
        if (checkTestFrameworks && (lower.contains("org.testng") || evidence.getTestFrameworkIndicators().contains("TestNG"))) {
        ...
        if (checkTestFrameworks && (lower.contains("org.junit") || lower.contains("assertionerror") ||
                lower.contains("tests run:") || evidence.getTestFrameworkIndicators().contains("JUnit"))) {
            return IssueCategory.JUNIT_TEST_FAILURE;
        }

        // Maven build failure
        if (checkBuildTools && (lower.contains("build failure") || lower.contains("[error]") ||
        ...
        if (checkBuildTools && (lower.contains("task :") || lower.contains("build failed") ||
